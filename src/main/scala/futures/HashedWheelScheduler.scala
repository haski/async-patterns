...
class HashedWheelScheduler extends Scheduler {
  ...
  override def scheduleOnce(initialDelay: FiniteDuration, task: => Unit): Unit =
    _scheduler.schedule(new Runnable { def run(): Unit = task }, initialDelay.toNanos, TimeUnit.NANOSECONDS)
}
